...
document.addEventListener('load', () => {
    ...
    if (video && video.duration) {
      ...
      setTimeout(() => {
        const skipBtn = document
          .querySelector('.videoAdUiSkipButton,.ytp-ad-skip-button,.ytp-ad-skip-button-modern');
        if (skipBtn) {
          skipBtn.click();
        }
      }, 50);
  ...
  const mastheadAd = document
    .querySelector('#masthead-ad > .style-scope.ytd-rich-grid-renderer');
